...
public class ProdutoServiceTest {
	...
	public void setup() {
		...
		prod = Produto.builder().id(null).categoria("Eletronico").nome("Geladeira").preco(2000.).qtdEstoque(100)
				.build();
	}
	...
	@Test
	public void givenFindProduct_when_thenReturn_Products() throws ObjectNotFoundException {
		
		List<Produto>produtosObj = new ArrayList<Produto>();
		produtosObj.add(prod);
		
		Mockito.when(produtoRepository.findAll()).thenReturn(produtosObj);
		
		List<Produto>produtos = produtoServiceImpl.buscarTodosProdutos();
